...
function draw() {
  ...
  text(
    "KNO3 Concentration: " + (result.KNO3_concentration ? result.KNO3_concentration.toFixed(3) : "Waiting for Input"),
    20,
    60
  );
  text(
    "Charcoal Concentration: " +
      (result.charcoal_concentration ? result.charcoal_concentration.toFixed(3) : "Waiting for Input"),
    20,
    80
  );
  text(
    "Sulfur Concentration: " +
      (result.sulfur_concentration ? result.sulfur_concentration.toFixed(3) :  "Waiting for Input"),
    20,
    100
  );
  text("Temperature: " + result.temperature.toFixed(3) + " K", 20, 120);
  text("Pressure: " + result.pressure.toFixed(3) + " Pa", 20, 140);
  text("Burn Rate: " + (result.burn_rate ? result.burn_rate.toFixed(3) + " ptcl/ts" :  "0 ptcl/ts"), 20, 160);
...
function drawLegend() {
  ...
  ellipse(width - 40, 60, calculateRadius("KNO3") * radiusMultipier, calculateRadius("KNO3") * radiusMultipier);
